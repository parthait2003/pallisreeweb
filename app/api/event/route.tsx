...
export async function POST(request: Request) {
  try {
    ...
    if (eventType === 'Camp') {
      ...
      page.drawText(`Camp Location: ${eventData.campLocation}`, {
        x: 50,
        y: 320,
        size: 14,
      });
      page.drawText(`Camp Name: ${eventData.campName}`, {
        x: 50,
        y: 300,
        size: 14,
      });
      page.drawText(`Camp Note: ${eventData.campNote || 'N/A'}`, {
        x: 50,
        y: 280,
        size: 14,
      });
      page.drawText(`Date: ${new Date(eventData.date).toLocaleDateString()}`, {
        x: 50,
        y: 260,
        size: 14,
      });
      page.drawText(`Time: ${new Date(eventData.time).toLocaleTimeString()}`, {
        x: 50,
        y: 240,
        size: 14,
      });
      page.drawText(`Trainee IDs: ${traineeIds.join(', ')}`, {
        x: 50,
        y: 220,
        size: 14,
      });
